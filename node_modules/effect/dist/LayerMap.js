...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (lookup, options) {
  ...
  const rcMap = yield* RcMap.make({
    lookup: key => Effect.servicesWith(_ => Layer.buildWithMemoMap(lookup(key), memoMap, ServiceMap.get(_, Scope.Scope))),
...
export const Service = () => (id, options) => {
  ...
  Object.setPrototypeOf(TagClass, Object.getPrototypeOf(ServiceMap.Service(id)));
  TagClass.key = id;
  ...
  TagClass_.services = key => Effect.flatMap(TagClass_.asEffect(), layerMap => layerMap.services(key));
