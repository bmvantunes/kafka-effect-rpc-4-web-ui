 ...
 * @category models
 */
export class CurrentMemoMap extends /*#__PURE__*/ServiceMap.Service()("effect/Layer/CurrentMemoMap") {
  static getOrCreate = /*#__PURE__*/ServiceMap.getOrElse(this, makeMemoMapUnsafe);
}
/**
 ...
 * @category memo map
 */
export const buildWithMemoMap = /*#__PURE__*/dual(3, (self, memoMap, scope) => internalEffect.provideService(internalEffect.map(self.build(memoMap, scope), ServiceMap.add(CurrentMemoMap, memoMap)), CurrentMemoMap, memoMap));
/**
 ...
 */
export const build = self => core.withFiber(fiber => buildWithMemoMap(self, CurrentMemoMap.getOrCreate(fiber.services), ServiceMap.getUnsafe(fiber.services, Scope.Scope)));
/**
 ...
 * @category destructors
 */
export const buildWithScope = /*#__PURE__*/dual(2, (self, scope) => core.withFiber(fiber => buildWithMemoMap(self, CurrentMemoMap.getOrCreate(fiber.services), scope)));
/**
...
export const succeed = function () {
  if (arguments.length === 1) {
    return resource => succeedServices(ServiceMap.make(arguments[0], resource));
  }
 ...
 * @category constructors
 */
export const succeedServices = services => fromBuildUnsafe(constant(internalEffect.succeed(services)));
/**
 ...
 * @category constructors
 */
export const empty = /*#__PURE__*/succeedServices(/*#__PURE__*/ServiceMap.empty());
/**
...
export const sync = function () {
  if (arguments.length === 1) {
    return evaluate => syncServices(() => ServiceMap.make(arguments[0], evaluate()));
  }
 ...
 * @category constructors
 */
export const syncServices = evaluate => fromBuildMemo(constant(internalEffect.sync(evaluate)));
/**
...
const effectImpl = (service, effect) => effectServices(internalEffect.map(effect, value => ServiceMap.make(service, value)));
/**
 ...
 * @category constructors
 */
export const effectServices = effect => fromBuildMemo((_, scope) => Scope.provide(effect, scope));
/**
 * Constructs a layer from the specified scoped effect.
 *
 ...
 * @category constructors
 */
export const effectDiscard = effect => effectServices(internalEffect.as(effect, ServiceMap.empty()));
/**
...
export const unwrap = self => {
  const service = ServiceMap.Service("effect/Layer/unwrap");
...
const mergeAllEffect = (layers, memoMap, scope) => {
  ...
  }).pipe(internalEffect.map(services => ServiceMap.mergeAll(...services)));
};
/**
...
const provideWith = (self, that, f) => fromBuild((memoMap, scope) => internalEffect.flatMap(Array.isArray(that) ? mergeAllEffect(that, memoMap, scope) : that.build(memoMap, scope), context => self.build(memoMap, scope).pipe(internalEffect.provideServices(context), internalEffect.map(merged => f(merged, context)))));
/**
 ...
 * @category utils
 */
export const provideMerge = /*#__PURE__*/dual(2, (self, that) => provideWith(self, that, (self, that) => ServiceMap.merge(that, self)));
/**
 ...
 * @category Testing
 */
export const mock = service => implementation => succeed(service)(new Proxy({
  ...implementation
}, {
 ...
 * @category tracing
 */
export const parentSpan = span => succeedServices(Tracer.ParentSpan.serviceMap(span));
/**
