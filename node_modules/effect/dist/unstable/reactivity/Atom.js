...
function makeEffect(ctx, effect, initialValue, services = ServiceMap.empty(), uninterruptible = false) {
  ...
  let syncResult;
  let isAsync = false;
  const cancel = runCallbackSync(ServiceMap.makeUnsafe(servicesMap), effect, function (exit) {
...
export const context = options => {
  ...
  const reactivityAtom = removeTtl(make(Effect.servicesWith(services => Layer.buildWithMemoMap(Reactivity.layer, options.memoMap, ServiceMap.get(services, Scope.Scope))).pipe(Effect.map(ServiceMap.get(Reactivity.Reactivity)))));
...
function makeStream(ctx, stream, initialValue, services = ServiceMap.empty()) {
  const previous = ctx.self();
  ...
  servicesMap.set(AtomRegistry.key, ctx.registry);
  servicesMap.set(Scheduler.Scheduler.key, ctx.registry.scheduler);
  const cancel = runCallbackSync(ServiceMap.makeUnsafe(servicesMap), run, constVoid, false);
  if (cancel !== undefined) {
    ctx.addFinalizer(cancel);
  }
...
const readSubscriptionRef = (get, sub, services = ServiceMap.empty()) => {
 ...
 * @category combinators
 */
export const transform = /*#__PURE__*/dual(2, (self, f) => isWritable(self) ? writable(get => f(get, self), function (ctx, value) {
  ctx.set(self, value);
...
export const swr = /*#__PURE__*/dual(2, (self, options) => {
  ...
  const refresh = self.refresh ?? function (f) {
    f(self);
  };
  function read(get) {
  ...
  return isWritable(self) ? writable(read, (ctx, value) => {
    ctx.set(self, value);
  }, refresh) : readable(read, refresh);
});
