 ...
 * @category handlers
 */
export const group = (api, groupName, build) => Layer.effectServices(Effect.gen(function* () {
  const services = yield* Effect.services();
 ...
 * @category handlers
 */
export const endpoint = (api, groupName, endpointName, handler) => Effect.servicesWith(services => {
  const group = api.groups[groupName];
  const endpoint = group.endpoints[endpointName];
  return Effect.succeed(handlerToHttpEffect(group, endpoint, services, handler, false));
});
/**
 * @since 4.0.0
 * @category security
...
function handlerToHttpEffect(group, endpoint, services, handler, isRaw) {
  ...
  return applyMiddleware(group, endpoint, services, Effect.gen(function* () {
  ...
  })).pipe(Effect.withErrorReporting, Effect.catch(error => Effect.orDie(encodeError(error))), Effect.provideServices(services));
}
function handlerToRoute(group, handler, services) {
  const endpoint = handler.endpoint;
  return HttpRouter.route(endpoint.method, endpoint.path, handlerToHttpEffect(group, endpoint, services, handler.handler, handler.isRaw), {
...
const applyMiddleware = (group, endpoint, services, handler) => {
  const options = {
    group,
    endpoint
  };
  for (const key_ of endpoint.middlewares) {
    const key = key_;
    const service = services.mapUnsafe.get(key_.key);
...
function getResponseEncode(status, encoding) {
