 ...
 * @category server
 */
export class McpServer extends /*#__PURE__*/ServiceMap.Service()("effect/ai/McpServer") {
  /**
   * @since 4.0.0
   */
...
export const run = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const clientMiddleware = McpServerClientMiddleware.of((effect, {
    clientId
  }) => Effect.provideService(effect, McpServerClient, McpServerClient.of({
    clientId,
    getClient: RcMap.get(clients, clientId).pipe(Effect.map(({
      client
    }) => client))
  })));
...
export const registerToolkit = /*#__PURE__*/Effect.fnUntraced(function* (toolkit) {
  ...
  for (const tool of Object.values(built.tools)) {
    ...
    yield* registry.addTool({
      tool: mcpTool,
      handle(payload) {
        return built.handle(tool.name, payload).pipe(Stream.unwrap, Stream.run(Sink.last()), Effect.flatMap(Effect.fromOption), Effect.provideServices(services), Effect.matchCause({
  ...
  if (arguments.length === 1) {
    ...
    return Effect.gen(function* () {
      ...
      yield* registry.addResource(new Resource({
        ...options,
        annotations: options
      }), options.content.pipe(Effect.provideServices(services), Effect.map(content => resolveResourceContent(options.uri, content)), Effect.catchCause(cause => {
    ...
    yield* registry.addPrompt({
      ...
      handle: params => decode(params).pipe(Effect.mapError(error => new InvalidParams({
        ...
        messages = typeof messages === "string" ? [{
          role: "user",
          content: TextContent.makeUnsafe({
            text: messages
          })
        }] : messages;
