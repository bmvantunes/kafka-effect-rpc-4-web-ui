...
import { CurrentCodecTransformer } from "./internal/codec-transformer.js";
...
const Proto = {
  ...
  asEffect() {
    return Effect.gen({
      ...
      const getSchemas = (tool, transformer) => {
        ...
        if (Predicate.isUndefined(schemas)) {
          ...
          const transformedResultSchema = Tool.isProviderDefined(tool) ? resultSchema : transformer(resultSchema).codec;
          ...
          const decodeResult = Schema.decodeUnknownEffect(transformedResultSchema);
          const encodeResult = Schema.encodeUnknownEffect(transformedResultSchema);
          schemas = {
      ...
      const handle = Effect.fnUntraced(function* (name, params) {
        ...
        const fiber = yield* schemas.handler(decodedParams, context).pipe(Effect.flatMap(result => Queue.offer(queue, {
          ...
          isFailure: false,
          preliminary: false
        })), Effect.updateServices(input => ServiceMap.merge(schemas.services, input)), Effect.matchCauseEffect({
