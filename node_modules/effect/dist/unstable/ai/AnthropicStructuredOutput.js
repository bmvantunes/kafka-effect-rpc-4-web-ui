 ...
 */
export function toCodecAnthropic(schema) {
  const to = schema.ast;
  const from = recur(AST.toEncoded(to));
  const codec = from === to ? schema : Schema.make(AST.decodeTo(from, to, Transformation.passthrough()));
...
function recur(ast) {
  switch (ast._tag) {
    ...
    case "Suspend":
      return unsupportedAst(ast, "Anthropic structured output does not support this schema kind; consider transforming the schema or using a different provider");
    case "Undefined":
      return unsupportedAst(ast, "Anthropic structured output does not support undefined; consider transforming the schema or using a different provider; if using `Schema.optional`, consider using `Schema.optionalKey` instead");
    case "Null":
      return ast;
    case "String":
      {
    ...
    case "Arrays":
      {
        ...
        if (ast.elements.length > 0) {
          ...
          return AST.decodeTo(recur(new AST.Objects(propertySignatures, [], annotations, filters)), ast, Transformation.transform({
            decode: o => {
              let t = [];
        ...
        if (ast.indexSignatures.length === 0) {
          const propertySignatures = AST.mapOrSame(ast.propertySignatures, ps => {
            ...
            if (AST.isOptional(ps.type)) {
              type = AST.decodeTo(new AST.Union([type, AST.null], "anyOf"), AST.optionalKey(type), Transformation.transformOptional({
          ...
          return AST.decodeTo(recur(new AST.Arrays(false, [], [new AST.Arrays(false, [is.parameter, is.type], [])], annotations)), ast, Transformation.transform({
...
const getChecks = ast => [...(ast.checks !== undefined ? getFilters(ast.checks) : []), ...getAnnotations(ast.annotations)];
...
//# sourceMappingURL=AnthropicStructuredOutput.js.map