 ...
 * @category services
 */
export class LanguageModel extends /*#__PURE__*/ServiceMap.Service()("effect/unstable/ai/LanguageModel") {}
/**
 ...
 * @category services
 */
export const CurrentCodecTransformer = InternalCodecTransformer.CurrentCodecTransformer;
/**
...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (params) {
  ...
  }), Effect.provideService(IdGenerator, idGenerator), Effect.provideService(InternalCodecTransformer.CurrentCodecTransformer, codecTransformer)));
  ...
  const streamText = Effect.fnUntraced(function* (options) {
    ...
    method: "streamText",
    reason: AiError.InvalidOutputError.fromSchemaError(error)
  }) : error), Stream.provideService(IdGenerator, idGenerator), Stream.provideService(InternalCodecTransformer.CurrentCodecTransformer, codecTransformer));
  ...
  const streamContent = Effect.fnUntraced(function* (options, providerOptions) {
    ...
    if (Predicate.isUndefined(options.toolkit)) {
      ...
      return params.streamText(providerOptions).pipe(Stream.mapArrayEffect(parts => decodeParts(parts)));
    }
    // If there is a toolkit resolve and apply it to the provider options
    ...
    const handleToolCall = part => Effect.gen(function* () {
    ...
    yield* params.streamText(providerOptions).pipe(Stream.runForEachArray(Effect.fnUntraced(function* (chunk) {
    ...
    // - fail (FiberSet.join)
    Effect.andThen(Effect.raceFirst(FiberSet.join(toolCallFibers), FiberSet.awaitEmpty(toolCallFibers))),
