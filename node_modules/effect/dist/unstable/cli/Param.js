...
export const optional = param => {
  const parse = args => param.parse(args).pipe(Effect.map(([leftover, value]) => [leftover, Option.some(value)]),
  // Catch both MissingOption (for flags) and MissingArgument (for positional arguments)
  Effect.catchTag("MissingOption", () => Effect.succeed([args.arguments, Option.none()])), Effect.catchTag("MissingArgument", () => Effect.succeed([args.arguments, Option.none()])));
  return Object.assign(Object.create(Proto), {
    _tag: "Optional",
    kind: param.kind,
    param,
...
export const withFallbackPrompt = /*#__PURE__*/dual(2, (self, prompt) => {
  const runPrompt = (error, args) => Prompt.run(prompt).pipe(Effect.map(value => [args.arguments, value]), Effect.catchTag("QuitError", () => Effect.fail(error)));
