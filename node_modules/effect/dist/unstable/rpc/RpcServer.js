...
export const makeNoSerialization = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const handleRequest = (requestFiber, client, request) => {
    ...
    if (enableTracing) {
      const parentSpan = requestFiber.services.mapUnsafe.get(Tracer.ParentSpan.key);
...
const applyMiddleware = (services, handler, options) => {
        ...
        case "Chunk":
          {
            ...
            return handleEncode(client, response.requestId, schemas.encodeDefect, schemas.collector, Effect.provideServices(schemas.encodeChunk(response.values), schemas.services), values => ({
              _tag: "Chunk",
        ...
        case "Exit":
          {
            ...
            return handleEncode(client, response.requestId, schemas.encodeDefect, schemas.collector, Effect.provideServices(schemas.encodeExit(response.exit), schemas.services), exit => ({
              _tag: "Exit",
              requestId: String(response.requestId),
              exit
            }));
  ...
  const getSchemas = rpc => {
    ...
    if (!schemas) {
      ...
      schemas = {
        ...
        encodeChunk: Schema.encodeUnknownEffect(Schema.toCodecJson(Schema.Array(streamSchemas ? streamSchemas.success : Schema.Any))),
 ...
 * @category protocol
 */
export class Protocol extends /*#__PURE__*/ServiceMap.Service()("effect/rpc/RpcServer/Protocol") {
  /**
   * @since 4.0.0
   */
