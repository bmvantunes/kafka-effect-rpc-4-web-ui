 ...
 * @since 4.0.0
 */
export const SqlClient = /*#__PURE__*/ServiceMap.Service("effect/sql/SqlClient");
/**
...
export const makeWithTransaction = options => effect => Effect.uninterruptibleMask(restore => Effect.useSpan("sql.transaction", {
  ...
  return Effect.flatMap(conn, ([scope, conn]) => (id === 0 ? options.begin(conn) : options.savepoint(conn, id)).pipe(Effect.flatMap(() => Effect.provideServices(restore(effect), ServiceMap.add(services, options.transactionService, [conn, id]).pipe(ServiceMap.add(Tracer.ParentSpan, span)))), Effect.exit, Effect.flatMap(exit => {
    let effect;
 ...
 * @since 4.0.0
 */
export class TransactionConnection extends /*#__PURE__*/ServiceMap.Service()("effect/sql/SqlClient/TransactionConnection") {}
/**
 * @since 4.0.0
 */
export const SafeIntegers = /*#__PURE__*/ServiceMap.Reference("effect/sql/SqlClient/SafeIntegers", {
  defaultValue: () => false
});
