...
export const request = function () {
  if (arguments.length === 1) {
    ...
    return payload => Effect.request(SqlRequest(payload), Effect.succeed(resolver));
  }
...
const partitionRequests = function* (requests, schema) {
  ...
  for (let i = 0; i < len; i++) {
    entry = requests[i];
    yield Effect.provideServices(handle(encode(entry.request.payload)), entry.services);
  }
  return inputs;
};
