...
select * from ${sql(options.tableName)} where ${sql(idColumn)} = LAST_INSERT_ID();`.unprepared.pipe(Effect.map(([, results]) => results[0])), {
  ...
  }).pipe(RequestResolver.setDelay(options.window), setMaxBatchSize, RequestResolver.withSpan(`${options.spanPrefix}.insertResolver`));
  const insertExecute = SqlResolver.request(insertResolver);
  const insert = insert => insertExecute(insert).pipe(Effect.catchTag("ResultLengthMismatch", Effect.die), Effect.withSpan(`${options.spanPrefix}.insert`, {}, {
    captureStackTrace: false
  }));
  const insertVoidResolver = SqlResolver.void({
    Request: Model.insert,
    execute: request => sql`insert into ${sql(options.tableName)} ${sql.insert(request)}`
  }).pipe(RequestResolver.setDelay(options.window), setMaxBatchSize, RequestResolver.withSpan(`${options.spanPrefix}.insertVoidResolver`));
  const insertVoidExecute = SqlResolver.request(insertVoidResolver);
  const insertVoid = insert => insertVoidExecute(insert).pipe(Effect.withSpan(`${options.spanPrefix}.insertVoid`, {}, {
    captureStackTrace: false
  }));
  ...
  }).pipe(RequestResolver.setDelay(options.window), setMaxBatchSize, RequestResolver.withSpan(`${options.spanPrefix}.findByIdResolver`));
  const findByIdExecute = SqlResolver.request(findByIdResolver);
  const findById = id => findByIdExecute(id).pipe(Effect.withSpan(`${options.spanPrefix}.findById`, {
    attributes: {
      id
    }
  }, {
  ...
  }).pipe(RequestResolver.setDelay(options.window), setMaxBatchSize, RequestResolver.withSpan(`${options.spanPrefix}.deleteResolver`));
  const deleteExecute = SqlResolver.request(deleteResolver);
  const delete_ = id => deleteExecute(id).pipe(Effect.withSpan(`${options.spanPrefix}.delete`, {
    attributes: {
      id
    }
  }, {
