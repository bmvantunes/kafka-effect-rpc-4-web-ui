 ...
 * @category HttpRouter
 */
export const HttpRouter = /*#__PURE__*/ServiceMap.Service("effect/http/HttpRouter");
/**
 * @since 4.0.0
 * @category HttpRouter
 */
export const make = /*#__PURE__*/Effect.gen(function* () {
  ...
  const addAll = routes => Effect.servicesWith(services => {
  ...
  return HttpRouter.of({
    ...
    asHttpEffect() {
      let handler = Effect.withFiber(fiber => {
        ...
        return Effect.provideServices(route.uninterruptible ? route.handler : Effect.interruptible(route.handler), ServiceMap.makeUnsafe(contextMap));
      });
 ...
 */
export const RouterConfig = /*#__PURE__*/ServiceMap.Reference("effect/http/HttpRouter/RouterConfig", {
  defaultValue: () => ({})
});
/**
 * @since 4.0.0
 ...
 */
export class RouteContext extends /*#__PURE__*/ServiceMap.Service()("effect/http/HttpRouter/RouteContext") {}
/**
 * @since 4.0.0
 * @category RouteContext
 */
...
export const prefixRoute = /*#__PURE__*/dual(2, (self, prefix) => makeRoute({
  ...
  prefix: UndefinedOr.match(self.prefix, {
    onUndefined: () => prefix,
    onDefined: existingPrefix => prefixPath(existingPrefix, prefix)
  })
}));
...
})) : new MiddlewareImpl(Effect.isEffect(middleware) ? Layer.effectServices(Effect.map(middleware, fn => ServiceMap.makeUnsafe(new Map([[fnContextKey, fn]])))) : Layer.succeedServices(ServiceMap.makeUnsafe(new Map([[fnContextKey, middleware]]))));
let middlewareId = 0;
...
class MiddlewareImpl {
  ...
  constructor(layerFn, dependencies) {
    ...
    this.layer = Layer.effectServices(Effect.gen({
      self: this
    }, function* () {
      ...
      return ServiceMap.makeUnsafe(new Map([[contextKey, stack]]));
...
export const provideRequest = layer => self => Layer.provide(self, middleware()(Effect.gen(function* () {
  ...
  return effect => Effect.provideServices(effect, services);
})).layer);
/**
