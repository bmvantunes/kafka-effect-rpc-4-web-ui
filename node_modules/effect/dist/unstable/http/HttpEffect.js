 ...
 */
export const toWebHandlerWith = services => (self, middleware) => {
  ...
  return (request, reqServices) => new Promise(resolve => {
    const contextMap = new Map(services.mapUnsafe);
    ...
    const fiber = Effect.runForkWith(ServiceMap.makeUnsafe(contextMap))(httpApp);
 ...
 * @category conversions
 */
export const toWebHandler = /*#__PURE__*/toWebHandlerWith(/*#__PURE__*/ServiceMap.empty());
/**
 * @since 4.0.0
 * @category conversions
 */
export const toWebHandlerLayerWith = (layer, options) => {
  ...
  let handlerCache;
  let handlerPromise;
  function handler(request, services) {
    if (handlerCache) {
      return handlerCache(request, services);
    }
    handlerPromise ??= Effect.runPromise(Effect.gen(function* () {
      const services = yield* options.memoMap ? Layer.buildWithMemoMap(layer, options.memoMap, scope) : Layer.buildWithScope(layer, scope);
      return handlerCache = toWebHandlerWith(services)(yield* options.toHandler(services), options.middleware);
    }));
