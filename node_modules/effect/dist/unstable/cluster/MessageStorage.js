 ...
 * @category context
 */
export class MessageStorage extends /*#__PURE__*/ServiceMap.Service()("effect/cluster/MessageStorage") {}
/**
 * @since 4.0.0
 * @category SaveResult
 */
...
export const makeEncoded = /*#__PURE__*/Effect.fnUntraced(function* (encoded) {
  ...
  const storage = yield* make({
    saveRequest: message => Message.serializeEnvelope(message).pipe(Effect.flatMap(envelope => encoded.saveEnvelope({
      ...
      return Schema.decodeEffect(schema)(result.lastReceivedReply).pipe(Effect.provideServices(message.services), MalformedMessage.refail, Effect.map(reply => SaveResult.Duplicate({
    ...
    unprocessedMessages: shardIds => {
      ...
      return Effect.flatMap(Effect.suspend(() => encoded.unprocessedMessages(shards, clock.currentTimeMillisUnsafe())), decodeMessages);
    },
    unprocessedMessagesById(messageIds) {
      const ids = Array.from(messageIds);
      if (!Arr.isArrayNonEmpty(ids)) return Effect.succeed([]);
      return Effect.flatMap(Effect.suspend(() => encoded.unprocessedMessagesById(ids, clock.currentTimeMillisUnsafe())), decodeMessages);
    },
  ...
  const decodeReplies = (messages, encodedReplies) => {
    ...
    const decodeReply = Effect.catch(Effect.suspend(() => {
      ...
      const schema = Reply.Reply(message.rpc);
      return Schema.decodeEffect(schema)(reply).pipe(Effect.provideServices(message.services));
    }), error => {
      const reply = encodedReplies[index];
 ...
 * @category Memory
 */
export class MemoryDriver extends /*#__PURE__*/ServiceMap.Service()("effect/cluster/MessageStorage/MemoryDriver", {
      ...
      saveEnvelope: ({
        ...
        if (existing) {
          return SaveResultEncoded.Duplicate({
            ...
            lastReceivedReply: existing.replies.length === 1 && existing.replies[0]._tag === "WithExit" ? existing.replies[0] : existing.lastReceivedChunk
          });
        }
        if (envelope._tag === "Request") {
          const entry = {
...
const EnvelopeWithReply = /*#__PURE__*/Schema.Struct({
  ...
  lastSentReply: /*#__PURE__*/Schema.UndefinedOr(Reply.Encoded)
});
