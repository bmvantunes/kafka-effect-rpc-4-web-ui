...
import { isUninterruptibleForServer, Persisted } from "../ClusterSchema.js";
...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (entity, buildHandlers, options) {
  ...
  const retryDriver = yield* Schedule.toStepWithSleep(options.defectRetryPolicy ? Schedule.andThen(options.defectRetryPolicy, defaultRetryPolicy) : defaultRetryPolicy);
  ...
  const entities = yield* ResourceMap.make(Effect.fnUntraced(function* (address) {
    ...
    const writeRef = yield* ResourceRef.from(scope, Effect.fnUntraced(function* (scope) {
      ...
      const handlers = yield* entity.protocol.toHandlers(buildHandlers).pipe(Effect.provideService(CurrentLogAnnotations, {}), Effect.provideServices(services.pipe(ServiceMap.add(CurrentAddress, address), ServiceMap.add(CurrentRunnerAddress, options.runnerAddress), ServiceMap.add(KeepAliveLatch, keepAliveLatch), ServiceMap.add(Scope.Scope, scope))));
            ...
            case "Exit":
              {
                ...
                // interrupt.
                if (storageEnabled && ServiceMap.get(request.rpc.annotations, Persisted) && Exit.hasInterrupts(response.exit) && (isShuttingDown || isUninterruptibleForServer(request.rpc.annotations))) {
                  if (!isShuttingDown) {
  ...
  const typeAttributes = Metric.CurrentMetricAttributes.serviceMap({
    type: entity.type
  });
  yield* Effect.sync(() => {
        ...
        case "IncomingEnvelope":
          {
            ...
            if (!entry) {
              return Effect.void;
            } else if (message.envelope._tag === "AckChunk" && entry.lastSentChunk !== undefined && message.envelope.replyId !== entry.lastSentChunk.id) {
              return Effect.void;
            }
...
const makeMessageDecode = (entity, entityRpcs) => {
  const decodeRequest = Effect.fnUntracedEager(function* (message, rpc) {
    ...
    const lastSentReply = message.lastSentReply !== undefined ? yield* Schema.decodeEffect(Reply.Reply(rpc))(message.lastSentReply) : undefined;
    return {
