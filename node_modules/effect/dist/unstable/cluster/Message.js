...
export const serializeRequest = self => {
  const rpc = self.rpc;
  return Schema.encodeEffect(Schema.toCodecJson(rpc.payloadSchema))(self.envelope.payload).pipe(Effect.provideServices(self.services), MalformedMessage.refail, Effect.map(payload => ({
    ...self.envelope,
    payload
  })));
};
...
export const deserializeLocal = (self, encoded) => {
  ...
  const rpc = self.rpc;
  return Schema.decodeEffect(Schema.toCodecJson(rpc.payloadSchema))(encoded.payload).pipe(Effect.provideServices(self.services), MalformedMessage.refail, Effect.map(payload => new IncomingRequestLocal({
