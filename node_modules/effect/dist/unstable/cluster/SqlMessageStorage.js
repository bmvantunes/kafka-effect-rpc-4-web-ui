...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  return yield* MessageStorage.makeEncoded({
    saveEnvelope: ({
      ...
      return insert.pipe(Effect.map(rows => {
        ...
        return SaveResultEncoded.Duplicate({
          ...
          lastReceivedReply: row.reply_id ? replyKindNum === replyKind.WithExit ? {
    ...
    requestIdForPrimaryKey: primaryKey => sql`SELECT id FROM ${messagesTableSql} WHERE message_id = ${primaryKey}`.pipe(Effect.map(rows => UndefinedOr.map(rows[0]?.id, Snowflake.Snowflake)), Effect.provideService(SqlClient.SafeIntegers, true), PersistenceError.refail, withTracerDisabled),
