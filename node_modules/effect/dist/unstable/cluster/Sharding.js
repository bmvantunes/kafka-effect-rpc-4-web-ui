 ...
 * @category models
 */
export class Sharding extends /*#__PURE__*/ServiceMap.Service()("effect/cluster/Sharding") {}
const make = /*#__PURE__*/Effect.gen(function* () {
  ...
  const isLocalRunner = address => Equal.equals(address, config.runnerAddress);
    ...
    return Effect.catchFilter(Effect.suspend(() => {
      ...
      if (isPersisted) {
        return runnerIsLocal ? notifyLocal(message, discard) : runnersService.notify({
          address: Option.getOrUndefined(maybeRunner),
          message,
          discard
        });
  ...
  const selfRunner = config.runnerAddress ? new Runner({
  ...
  yield* Effect.gen(function* () {
    ...
    while (true) {
      ...
      if (changed) {
        ...
        // update metrics
        if (selfRunner) {
          ClusterMetrics.runnersHealthy.updateUnsafe(BigInt(MutableHashSet.has(healthyRunners, selfRunner) ? 1 : 0), ServiceMap.empty());
        }
      }
  ...
  const registerSingleton = Effect.fnUntraced(function* (name, run, options) {
    ...
    const wrappedRun = run.pipe(Effect.andThen(Effect.never), Effect.scoped, Effect.provideService(CurrentLogAnnotations, {}), Effect.provideServices(services), Effect.orDie, Effect.interruptible);
  ...
  const syncSingletons = withSingletonLock(Effect.gen(function* () {
    ...
    ClusterMetrics.singletons.updateUnsafe(BigInt(yield* FiberMap.size(singletonFibers)), ServiceMap.empty());
  }));
  // --- Entities ---
  ...
  const registerEntity = Effect.fnUntraced(function* (entity, build, options) {
    ...
    const manager = yield* EntityManager.make(entity, build, {
      ...
      sharding
    }).pipe(Effect.provideServices(services.pipe(ServiceMap.add(EntityReaper, reaper), ServiceMap.add(Scope.Scope, scope), ServiceMap.add(Snowflake.Generator, snowflakeGen))));
    const state = {
      entity,
      status: "alive",
      manager
...
// Utilities
const ClientAddressTag = /*#__PURE__*/ServiceMap.Service("effect/cluster/Sharding/ClientAddress");
