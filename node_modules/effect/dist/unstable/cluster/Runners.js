 ...
 * @category context
 */
export class Runners extends /*#__PURE__*/ServiceMap.Service()("effect/cluster/Runners") {}
/**
 * @since 1.0.0
    ...
    send({
      ...
      if (!isStream) {
        return Effect.matchEffect(Message.serializeRequest(message), {
          onSuccess: request => RcMap.get(clients, address).pipe(Effect.flatMap(client => client.Effect({
            request,
            persisted: isPersisted
          })), Effect.catchTag("RpcClientError", Effect.die), Effect.flatMap(reply => Schema.decodeEffect(Reply.Reply(message.rpc))(reply).pipe(Effect.provideServices(message.services), Effect.orDie)), Effect.flatMap(message.respond), Effect.scoped, Effect.catchDefect(() => Effect.fail(new RunnerUnavailable({
            address
          })))),
      ...
      return Effect.matchEffect(Message.serializeRequest(message), {
        onSuccess: request => RcMap.get(clients, address).pipe(Effect.flatMap(client => client.Stream({
          ...
          return Queue.take(queue).pipe(Effect.flatMap(reply => Effect.orDie(decode(reply))), Effect.flatMap(message.respond), Effect.forever, Effect.catchTag("RpcClientError", Effect.die), Effect.provideServices(message.services), Effect.catchTag("Done", _ => Effect.void), Effect.catchDefect(() => Effect.fail(new RunnerUnavailable({
            address
          }))));
        }), Effect.scoped),
        onFailure: error => message.respond(new Reply.WithExit({
 ...
 * @category Client
 */
export class RpcClientProtocol extends /*#__PURE__*/ServiceMap.Service()("effect/cluster/Runners/RpcClientProtocol") {}
