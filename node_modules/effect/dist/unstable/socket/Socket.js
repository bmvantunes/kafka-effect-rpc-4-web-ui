 ...
 * @category tags
 */
export const Socket = /*#__PURE__*/ServiceMap.Service("effect/socket/Socket");
 ...
 * @category errors
 */
export class SocketError extends /*#__PURE__*/Schema.ErrorClass(SocketErrorTypeId)({
 ...
 * @category tags
 */
export class WebSocket extends /*#__PURE__*/ServiceMap.Service()("~effect/socket/Socket/WebSocket") {}
/**
 * @since 4.0.0
 * @category tags
 */
export class WebSocketConstructor extends /*#__PURE__*/ServiceMap.Service()("@effect/platform/Socket/WebSocketConstructor") {}
/**
 * @since 4.0.0
 * @category layers
 */
 ...
 * @category constructors
 */
export const makeWebSocket = (url, options) => fromWebSocket(Effect.acquireRelease((typeof url === "string" ? Effect.succeed(url) : url).pipe(Effect.flatMap(url => Effect.map(WebSocketConstructor.asEffect(), f => f(url, options?.protocols)))), ws => Effect.sync(() => ws.close(1000))), options);
/**
 * @since 4.0.0
 * @category constructors
 */
export const fromWebSocket = (acquire, options) => Effect.withFiber(fiber => {
  ...
  })).pipe(Effect.updateServices(input => ServiceMap.merge(acquireContext, input)), Effect.ensuring(Effect.sync(() => {
    latch.closeUnsafe();
    currentWS = undefined;
  })));
  const encoder = new TextEncoder();
 ...
 * @category fiber refs
 */
export const SendQueueCapacity = /*#__PURE__*/ServiceMap.Reference("~effect/socket/Socket/SendQueueCapacity", {
  defaultValue: () => 16
});
/**
 * @since 4.0.0
...
export const fromTransformStream = (acquire, options) => Effect.withFiber(fiber => {
  const latch = Latch.makeUnsafe(false);
  let currentStream;
  const acquireServices = fiber.services;
  const closeCodeIsError = options?.closeCodeIsError ?? defaultCloseCodeIsError;
  const runRaw = (handler, opts) => Effect.scopedWith(Effect.fnUntraced(function* (scope) {
    ...
    if (opts?.onOpen) yield* opts.onOpen;
    return yield* Effect.catchFilter(FiberSet.join(fiberSet), SocketCloseError.filterClean(_ => !closeCodeIsError(_)), () => Effect.void);
  })).pipe(_ => _, Effect.updateServices(input => ServiceMap.merge(acquireServices, input)), Effect.ensuring(Effect.sync(() => {
    latch.closeUnsafe();
