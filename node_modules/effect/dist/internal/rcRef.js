...
class RcRefImpl {
  ...
  scope;
  idleTimeToLive;
  constructor(acquire, services, scope, idleTimeToLive) {
...
export const make = options => Effect.withFiber(fiber => {
  ...
  const ref = new RcRefImpl(options.acquire, services, scope, options.idleTimeToLive ? Duration.fromInputUnsafe(options.idleTimeToLive) : undefined);
    ...
    case "Empty":
      {
        ...
        return self.semaphore.withPermits(1)(restore(Effect.provideServices(self.acquire, ServiceMap.add(self.services, Scope.Scope, scope))).pipe(Effect.map(value => {
          const state = {
            _tag: "Acquired",
            value,
            scope,
...
export const get = /*#__PURE__*/Effect.fnUntraced(function* (self_) {
  ...
  yield* Scope.addFinalizerExit(scope, () => {
    ...
    })), Effect.runForkWith(self.services), Fiber.runIn(self.scope));
    return Effect.void;
  });
  return state.value;
});
