 ...
 * @category getters
 */
export const getNode = /*#__PURE__*/dual(2, (graph, nodeIndex) => graph.nodes.has(nodeIndex) ? Option.some(graph.nodes.get(nodeIndex)) : Option.none());
/**
...
export const updateEdge = (mutable, edgeIndex, f) => {
  ...
  mutable.edges.set(edgeIndex, new Edge({
    ...currentEdge,
    data: newData
  }));
};
...
export const reverse = mutable => {
  ...
  for (const [index, edgeData] of mutable.edges) {
    mutable.edges.set(index, new Edge({
 ...
 * @category getters
 */
export const getEdge = /*#__PURE__*/dual(2, (graph, edgeIndex) => graph.edges.get(edgeIndex));
/**
 ...
 * @category getters
 */
export const neighbors = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
...
export const astar = /*#__PURE__*/dual(2, (graph, config) => {
  ...
  const openSet = [{
    node: config.source,
    fScore: fScore.get(config.source)
  }];
  ...
  while (currentNode !== null) {
    path.unshift(currentNode);
    const prev = previous.get(currentNode) ?? null;
    if (prev !== null) {
      costs.unshift(prev.edgeData);
      currentNode = prev.node;
    } else {
 ...
 * @category iterators
 */
export const topo = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, config = {}) => {
 ...
 * @category iterators
 */
export const externals = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, config = {}) => {
  const direction = config.direction ?? "outgoing";
  return new Walker(f => ({
    [Symbol.iterator]: () => {
      const nodeMap = graph.nodes;
