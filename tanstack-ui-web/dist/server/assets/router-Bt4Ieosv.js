import { useRouterState, Link, createRootRoute, Outlet, HeadContent, Scripts, createFileRoute, lazyRouteComponent, createRouter } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import { Schema, Effect, Stream, Exit } from "effect";
import { RpcGroup, Rpc, RpcClient, RpcSerialization } from "effect/unstable/rpc";
...
const HeartbeatGroupDimensionSchema = Schema.Union([
...
const HeartbeatGroupFiltersSchema = Schema.Struct({
...
const HeartbeatGroupQuerySchema = Schema.Struct({
...
const HeartbeatGroupChildSchema = Schema.Struct({
  key: Schema.String,
  minTimestamp: Schema.Number,
  maxTimestamp: Schema.Number,
  count: Schema.Number
});
const HeartbeatGroupSchema = Schema.Struct({
  key: Schema.String,
  minTimestamp: Schema.Number,
  maxTimestamp: Schema.Number,
  count: Schema.Number,
...
const HeartbeatGroupSnapshotEventSchema = Schema.Struct({
  type: Schema.Literal("snapshot"),
  emittedAt: Schema.Number,
  groups: Schema.Array(HeartbeatGroupSchema)
});
const HeartbeatGroupUpsertEventSchema = Schema.Struct({
  type: Schema.Literal("upsert"),
  emittedAt: Schema.Number,
  group: HeartbeatGroupSchema
});
const HeartbeatGroupDeleteEventSchema = Schema.Struct({
  type: Schema.Literal("delete"),
  emittedAt: Schema.Number,
  key: Schema.String
});
const HeartbeatGroupEventSchema = Schema.Union([
...
const SubscribeGroupsRpc = Rpc.make("SubscribeGroups", {
  payload: HeartbeatGroupQuerySchema,
  success: HeartbeatGroupEventSchema,
  stream: true
});
const HeartbeatRpcGroup = RpcGroup.make(SubscribeGroupsRpc);
...
const HeartbeatEndpointContext = createContext(defaultValue);
const useHeartbeatEndpoint = () => useContext(HeartbeatEndpointContext);
...
const useTick = () => useSyncExternalStore(
  tickStore.subscribe,
...
const MetricCard = ({
  ...
  value,
  caption
}) => /* @__PURE__ */ jsxs("article", { className: "rounded-[1.6rem] border border-white/12 bg-white/6 p-5 shadow-[0_24px_80px_rgba(4,8,20,0.22)] backdrop-blur", children: [
  /* @__PURE__ */ jsx("p", { className: "text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-400", children: eyebrow }),
  /* @__PURE__ */ jsx("p", { className: "mt-3 text-3xl font-semibold text-white", children: value }),
  /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm text-slate-300", children: caption })
] });
const Panel = ({
  kicker,
  title,
  ...
  actions,
  children
}) => /* @__PURE__ */ jsxs("section", { className: "rounded-[2rem] border border-white/12 bg-slate-950/72 p-6 shadow-[0_24px_100px_rgba(3,7,18,0.32)] backdrop-blur-xl md:p-7", children: [
  /* @__PURE__ */ jsxs("header", { className: "flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between", children: [
    /* @__PURE__ */ jsxs("div", { className: "max-w-2xl", children: [
      /* @__PURE__ */ jsx("p", { className: "text-[11px] font-semibold uppercase tracking-[0.36em] text-cyan-200/70", children: kicker }),
      /* @__PURE__ */ jsx("h2", { className: "mt-2 text-2xl font-semibold text-white", children: title }),
      /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm leading-6 text-slate-300", children: description })
    ] }),
    actions
  ] }),
...
const EmptyPanel = ({
  title,
  body
}) => /* @__PURE__ */ jsxs("div", { className: "rounded-[1.6rem] border border-dashed border-white/16 bg-white/4 px-6 py-10 text-center", children: [
  /* @__PURE__ */ jsx("h3", { className: "text-lg font-semibold text-white", children: title }),
  /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm text-slate-300", children: body })
] });
const navigation = [
...
const MonitorShell = ({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(34,197,94,0.12),_transparent_24%),radial-gradient(circle_at_right,_rgba(56,189,248,0.14),_transparent_28%),linear-gradient(180deg,_#08111f_0%,_#060b16_42%,_#050910_100%)] text-slate-100", children: [
    /* @__PURE__ */ jsx("div", { className: "pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,transparent_0%,rgba(255,255,255,0.04)_50%,transparent_100%)] opacity-40" }),
    /* @__PURE__ */ jsx("div", { className: "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(251,191,36,0.12),transparent_18%),radial-gradient(circle_at_80%_12%,rgba(14,165,233,0.18),transparent_20%),radial-gradient(circle_at_50%_80%,rgba(34,197,94,0.12),transparent_24%)]" }),
    /* @__PURE__ */ jsxs("div", { className: "relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-12 pt-5 sm:px-6 lg:px-8", children: [
      /* @__PURE__ */ jsxs("header", { className: "sticky top-4 z-20 mb-8 rounded-[2rem] border border-white/12 bg-slate-950/78 px-5 py-4 shadow-[0_26px_90px_rgba(3,7,18,0.36)] backdrop-blur-xl", children: [
          ...
          /* @__PURE__ */ jsxs("div", { children: [
            /* @__PURE__ */ jsxs(
              Link,
              {
                ...
                children: [
                  /* @__PURE__ */ jsx("span", { className: "inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-emerald-300/30 bg-emerald-400/12 text-sm font-bold uppercase text-emerald-100", children: "PM" }),
                  "Pulse Monitor"
                ]
              }
            ),
            /* @__PURE__ */ jsx("p", { className: "mt-2 max-w-2xl text-sm leading-6 text-slate-300", children: "Live heartbeat supervision for cross-region systems. Regions stay green only while every observed system, app, host and process inside them is fresh. Systems stay green only while every observed app, region, host and process under them is fresh." })
          ] }),
          /* @__PURE__ */ jsxs("div", { className: "flex flex-col gap-3 text-sm text-slate-300", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex flex-wrap items-center justify-end gap-3", children: [
              /* @__PURE__ */ jsx(ConnectionPill, { status: gmailBadge.status }),
              /* @__PURE__ */ jsx(StatusPill, { label: gmailLabel, tone: gmailBadge.tone }),
              /* @__PURE__ */ jsx("span", { className: "rounded-full border border-white/12 bg-white/6 px-3 py-1 font-mono text-[11px] text-slate-200", children: wsUrl })
            ] }),
            /* @__PURE__ */ jsxs("p", { className: "text-right text-xs uppercase tracking-[0.28em] text-slate-400", children: [
              "trader badge source ",
              gmailBadge.system,
              " ",
        ...
        /* @__PURE__ */ jsx("nav", { className: "mt-5 flex flex-wrap gap-3", children: navigation.map((item) => {
...
function RootComponent() {
  return /* @__PURE__ */ jsx(RootDocument, { children: /* @__PURE__ */ jsx(MonitorShell, { children: /* @__PURE__ */ jsx(Outlet, {}) }) });
}
...
const $$splitComponentImporter$5 = () => import("./systems-C-yB5yit.js");
const Route$5 = createFileRoute("/systems")({
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("./regions-Cyniy7SP.js");
const Route$4 = createFileRoute("/regions")({
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./process-names-CESaqiri.js");
const Route$3 = createFileRoute("/process-names")({
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("./hostnames-CQYRVpEI.js");
const Route$2 = createFileRoute("/hostnames")({
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("./appnames--ZpYvCIW.js");
const Route$1 = createFileRoute("/appnames")({
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("./index-BkBW05hY.js");
